...
class ResNet50BEG(ResNet50BothEyes):
    def __init__(self, num_classes, pretrained, hidden_units, hidden_layers, dropout, freeze_layers, **kwargs):
        super().__init__(num_classes, pretrained, hidden_units, hidden_layers, dropout, freeze_layers)

    def get_base_model(self, run_id: str) -> nn.Module:
        
        model_aux = mlflow.pytorch.load_model(run_id)
        model = model_aux.model
        linear_in = nn.Sequential(nn.Linear(2048 * 2, self.hidden_units), nn.ReLU())
        return model, linear_in
        
model = ResNet50BEG(num_classes=config["num_classes"],
